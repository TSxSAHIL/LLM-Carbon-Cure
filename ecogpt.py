...
def main():
    ...
    page = st.sidebar.selectbox("Choose a page", ["Home Page", "Industrial Carbon Emissions", "Vehicle Carbon Emissions", "Predict CO2 Emissions", "ECO - GPT 🍀"])
    ...
    if page == "Home Page":
    ...
    elif page == "Industrial Carbon Emissions":
    ...
    elif page == "Vehicle Carbon Emissions":
        ...
        st.subheader('ECO - GPT 🍀')
        st.sidebar.text('Welcome to ECO - GPT 🍀')
        st.sidebar.write('ECOgpt is an interactive AI chatbot designed to provide insights and answers on carbon emissions and sustainability. It leverages advanced GPT models to understand and respond to user queries, offering a conversational interface for exploring topics related to CO2 emissions, renewable energy, and digital sustainability.')
        st.text('Ask about CO2 emissions and get an instant response!')
    ...
    #     # Create a chat message container for the user's input
    #     with st.chat_message("user"):
    #         user_input = st.text_input('')

    #     # Display a chat input widget for the user's question
    #     if st.button("Submit"):
    #         inputs = tokenizer.encode(user_input, return_tensors="pt")
    #         outputs = model.generate(inputs, max_length=1500, num_return_sequences=1, no_repeat_ngram_size=2)
    #         response = tokenizer.decode(outputs[0], skip_special_tokens=True)

    #         # Create a chat message container for the model's response
